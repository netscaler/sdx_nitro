...
class br_snmpmanager_response extends base_response
{
	public br_snmpmanager[] br_snmpmanager;
}

class br_snmpmanager_responses extends base_response
{
	public br_snmpmanager_response[] br_snmpmanager_response_array;
}
...
public class br_snmpmanager extends base_resource
{
	 ...
	 * </pre>
	 */
	public static br_snmpmanager addmanager(nitro_service client, br_snmpmanager resource) throws Exception
	{
		return ((br_snmpmanager[]) resource.perform_operation(client, "addmanager"))[0];
	}
	 ...
	 * </pre>
	 */
	public static br_snmpmanager[] addmanager(nitro_service client, br_snmpmanager[] resources) throws Exception
	{
		if(resources == null)
		...
		if(resources.length == 1)
			return ((br_snmpmanager[]) resources[0].perform_operation(client, "addmanager"));
		
		return ((br_snmpmanager[]) perform_operation_bulk_request(client, resources, "addmanager"));
	}
	...
	{
		br_snmpmanager_response result = (br_snmpmanager_response) service.get_payload_formatter().string_to_resource(br_snmpmanager_response.class, response);
	...
	{
		br_snmpmanager_responses result = (br_snmpmanager_responses) service.get_payload_formatter().string_to_resource(br_snmpmanager_responses.class, response);
		if(result.errorcode != 0)
		{
			if (result.errorcode == SESSION_NOT_EXISTS)
				service.clear_session();
			throw new nitro_exception(result.message, result.errorcode, (base_response [])result.br_snmpmanager_response_array);
		}
		br_snmpmanager[] result_br_snmpmanager = new br_snmpmanager[result.br_snmpmanager_response_array.length];
		
		for(int i = 0; i < result.br_snmpmanager_response_array.length; i++)
		{
			result_br_snmpmanager[i] = result.br_snmpmanager_response_array[i].br_snmpmanager[0];
		}
		
		return result_br_snmpmanager;
	}
